...
export class ImageProcessor {
	...
	async validateAdaptiveLayers(foregroundPath, backgroundPath, monochromePath) {
		...
		try {
			...
			// Validate background layer (can be image or hex color)
			if (!backgroundPath) {
				return {
					isValid: false,
					error: "Background layer is required for adaptive icons",
				};
			}

			// Check if background is a hex color
			if (this.isHexColor(backgroundPath)) {
	 ...
	 */
	async prepareAdaptiveLayer(layerPath, targetSize) {
		...
		// Single high-quality resize operation to maintain best quality
		// Using 'contain' fits the image within target size while maintaining aspect ratio
		// Creates exact targetSize x targetSize canvas with transparent background
