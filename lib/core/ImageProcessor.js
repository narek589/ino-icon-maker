...
export class ImageProcessor {
	...
	async prepareAdaptiveLayer(
		...
		if (isForeground) {
			...
			const fgScale = platform === Platform.IOS ? this.fgScaleIOS : this.fgScaleAndroid;
			// Clamp to reasonable range (0.1 to 1.0) after applying scale
			safeZoneRatio = Math.min(1.0, Math.max(0.1, safeZoneRatio * fgScale));
			
			const safeZoneSize = Math.round(targetSize * safeZoneRatio);

			// Calculate padding needed - use floor to prevent rounding errors
			// that would make the total size exceed targetSize
			const totalPadding = targetSize - safeZoneSize;
			const paddingTop = Math.floor(totalPadding / 2);
			const paddingBottom = totalPadding - paddingTop; // This handles odd numbers correctly

			// Resize image to fit within the safe zone, then extend to exact targetSize with transparent padding
			return image
				.clone()
