...
export class ImageProcessor {
	...
	async prepareAdaptiveLayer(layerPath, targetSize, isForeground = false) {
		...
		// - Masking zone: 21dp on each side (42dp total)
		// - Safe zone = 66/108 = 0.6111 (61.11% content)
		// - Padding = 42/108 = 0.3889 (38.89% padding, ~19.44% on each side)
		// This ensures important content isn't clipped by circular/squircle launcher masks
		if (isForeground) {
			const paddingPercent = 42 / 108; // 0.3889 = 38.89% total padding per Android spec
			const innerSize = Math.round(targetSize * (1 - paddingPercent));

			// Resize image to fit within safe zone with transparent padding
