...
export function createConfigLines(config) {
	...
	// Show foreground scale if set
	if (config.fgScale && parseFloat(config.fgScale) !== 1.0) {
		lines.push(
			chalk.gray("  FG Content Scale:") + " " + chalk.cyan(`${config.fgScale}x`) + 
			chalk.gray(` (${parseFloat(config.fgScale) > 1 ? 'zoom in' : 'zoom out'})`)
		);
	}
	if (config.fgScaleIos && parseFloat(config.fgScaleIos) !== 1.0) {
		lines.push(
			chalk.gray("  iOS FG Scale:    ") + " " + chalk.cyan(`${config.fgScaleIos}x`)
		);
	}
	if (config.fgScaleAndroid && parseFloat(config.fgScaleAndroid) !== 1.0) {
		lines.push(
			chalk.gray("  Android FG Scale:") + " " + chalk.cyan(`${config.fgScaleAndroid}x`)
		);
	}
