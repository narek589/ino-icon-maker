...
export class HttpServer {
	...
	async handleGeneration(req, res) {
		...
		try {
			...
			// Step 6: Validate custom sizes if provided
			const customSizesValidation = this.validator.validateCustomSizes(
				req.query,
				req.body
			);
			if (!customSizesValidation.valid) {
				await this.generationHandler.cleanupFiles(uploadedFiles);
				return this.responseHandler.sendError(
					res,
					400,
					customSizesValidation.error
				);
			}
			const { customSizes } = customSizesValidation;

			if (customSizes) {
				console.log(`\n🎨 Custom size configuration applied`);
			}

			// Step 7: Create temporary output directory
			tempOutputDir = await this.generationHandler.createTempOutputDir();

			// Step 8: Build generation options
			...
			// Step 10: Clean up uploaded files
			await this.generationHandler.cleanupFiles(uploadedFiles);

			// Step 11: Send response with cleanup callback
